    ...
    */
    ...
    $(document).on("mousewheel DOMMouseScroll",function(e){
        ...
        */
        ...
        3. 대소문자 구분없이 찾을때 i라는 플러그뭄ㄴ자를 사용함!!!
        예) /,/g -> 모든 콤마를 찾아라!
            /Firefox/i -> 모든"Firefox"라는 문자를 대소문자 관계없이 찾아라!!!
        */
        ...
        // offset().top은 현제 선택요소의 top위치값을 숫자로 리턴함!!!
        
        // 기존  위치값 읽어오기 변경!!!!!
        // 전체 윈도우 높이값(winH)에 페이지 번호를 곱한다
         let pgpos = winH * pno;
        // 이렇게 하는 이유? 페이지 위치값이 구해졌지만 이동시 sync문제(일치)문제로 값이 잘 반영되지 않을때 원페이지의 위치 이동을 이렇게 계산해서 이동함!!!!
