...
//파라미터로 넘어온 아아템 명으로 페에지 셋팅값을 객체로 셋팅한다!!!

// pm값이 '디바이스'/'패션'일 경우 처리

if(pm==='디바이스') pm = '스마트폰';
else if(pm==='패션') pm = '가방';
// 카테고리 명은 첫번째 아이템으러 셋팅되도록함!!!
...
// 아이템 객체
    var item = {
        '스마트폰':{
            ...
            // 쌍따옴표 안에 쌍따옴표쓰는 방법!!!!
            // 내부의 쌍따옴표앞에 역슬래쉬 (\)
            // 역슬래쉬뒤의 문자는 특수문자 처리하여 표시됨!!
            // 코딩시 역슬래쉬(\)표시가 달라지는 이유는 글씨체에 따라서 달라짐!!!
        },
        '태블릿PC':{
            "아이콘":"ico2",
            "제목":"아이패드",
...
$(function(){
    ...
    // 만약 '태블릿PC이면 이미지 width를 70%로 조정함!!
    if(pm==='태블릿PC') img.css({width:'75%'})
